...
Public Class Form_Invoice
    ...
    Public pubIDCustomer As String
    Public pubNamaCustomer As String
...
#Region "Variable Reference"
  ...
  Dim pSahSplit() As String
  Dim pSahpArtB As String = ""
  Dim pSahpArt As String = ""
  Dim sPlitArt As String = ""
...
#Region "Initialize & Object Function"
    ...
    Private Sub CheckPermission(ByVal TypeUID As String, ByVal DataExist As Boolean)
        ...
        If MainPage.InvoiceApplication = True And pubIDCustomer <> Nothing Then
            TMPRecord = MyDatabase.MyReader("SELECT * FROM USERCATDT WHERE USERCATUID = '" & TypeUID & "' AND USERCATMODULETYPEID IN ('2215')")
        Else
    ...
    Public Sub GetDTOrder(ByVal MBUID As String)
        ...
        '    End While
        'End With
        'susilo 4 feb 2015. Dirubah karena ada modifier
        OrderListCollection.Clear()
        With OrderListCollection
            Dim ItemRecord As FbDataReader
    ...
    Private Sub CategoryList(ByVal Position As Long)
        ...
        If CatCollection.Count > 12 Then
            LoopFrom = Position
            If Position + 11 > CatCollection.Count Then
        ...
        If Position > 0 Then
            For i As Integer = LoopFrom To LoopTo
                ...
                CurOBJIndex = IIf(i > 12, (i Mod 12), i)
                CurOBJIndex = IIf(CurOBJIndex = 0, 12, CurOBJIndex)
        ...
        BackCategory.Enabled = Position > 12
        NextCategory.Enabled = CatCollection.Count > Position + 11
    ...
    Private Sub PMList(ByVal Position As Long)
        ...
        If PMCollection.Count > 28 Then
            LoopFrom = Position
            If Position + 27 > PMCollection.Count Then
        ...
        If Position > 0 Then
            For i As Integer = LoopFrom To LoopTo
                ...
                CurOBJIndex = IIf(i > 28, (i Mod 28), i)
                CurOBJIndex = IIf(CurOBJIndex = 0, 28, CurOBJIndex)
        ...
        BackItem.Enabled = Position > 28
        NextItem.Enabled = PMCollection.Count > Position + 27
    ...
    Private Sub CategoryClick(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Category1.Click, Category2.Click, Category3.Click, Category4.Click, Category5.Click, Category6.Click, Category7.Click, Category8.Click, Category9.Click, Category10.Click, Category11.Click, Category12.Click
    Me.Cursor = Cursors.WaitCursor
    ...
    If UserInformation.UserDeptUID = "1" Then
      VSql = "SELECT A.INVENUID, A.INVENNO, A.INVENNAME, A.INVENLEVEL, A.INVENTYPEID, A.INVENCATUID, A.INVENPARENTUID, A.INVENMEASUNITUID, A.INVENDFTWAREHUID, A.INVENDISCPERC, A.INVENSALESTAXUID, A.INVENDFTVENDUID, A.INVENPURCHTAXUID, A.INVENMINQTYREORDER, A.INVENDESC, " & _
              "A.INVENDFTPRICELISTLVL, A.INVENMEASUNITDESC, A.INVENKITCHENUID, A.INVENACTV, A.INVENDISPLAYORDER, A.INVENISPRINTED, B.INVENPRICELISTLVL1,B.INVENPRICELISTLVL2,B.INVENPRICELISTLVL3,B.INVENPRICELISTLVL4,B.INVENPRICELISTLVL5,B.INVENPRICELISTLVL6,B.INVENPRICELISTLVL7," & _
    ...
    Else
      VSql = "SELECT A.INVENUID, A.INVENNO, A.INVENNAME, A.INVENLEVEL, A.INVENTYPEID, A.INVENCATUID, A.INVENPARENTUID, A.INVENMEASUNITUID, A.INVENDFTWAREHUID, A.INVENDISCPERC, A.INVENSALESTAXUID, A.INVENDFTVENDUID, A.INVENPURCHTAXUID, A.INVENMINQTYREORDER, A.INVENDESC, " & _
              ...
              "LEFT JOIN (SELECT COUNT(INVENPARENTUID) AS jmlRowC,INVENPARENTUID AS idParent FROM INVEN GROUP BY idParent) AS C ON A.INVENUID=C.idParent " & _
              "LEFT JOIN (SELECT COUNT(INVENUID) AS jmlRowD,INVENUID AS idItem FROM INVENITEMMOD GROUP BY idItem) AS D ON A.INVENUID=D.idItem " & _
              "WHERE (A.INVENPARENTUID IS NULL OR A.INVENPARENTUID=A.INVENUID) AND A.INVENCATUID = '" & sender.tag & "' AND (A.INVENACTV IS NULL OR A.INVENACTV = 0 ) AND A.INVENDEPTUID='" & ReplacePetik(UserInformation.UserDeptUID) & "' ORDER BY A.INVENNAME"
    End If
    ...
    Private Sub ItemClick(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Item01.Click, Item02.Click, Item03.Click, Item04.Click, Item05.Click, Item06.Click, Item07.Click, Item08.Click, Item09.Click, Item10.Click, Item11.Click, Item12.Click, Item13.Click, Item14.Click, Item15.Click, Item16.Click, Item17.Click, Item18.Click, Item19.Click, Item20.Click, Item21.Click, Item22.Click, Item23.Click, Item24.Click, Item25.Click, Item26.Click, Item27.Click, Item28.Click
        ...
        If UBound(arrayData) >= 5 Then
            ...
            If CInt(arrayData(7)) = 0 And CInt(arrayData(8)) = 0 Then
              ...
              ItemIdentifyClick = "plus"
              Call TranfToCustDisplay()
            ...
            If .Rows.Count > 1 Then
                Select Case sender.name
                    Case "QTYPlus"
                        ...
                        Application.DoEvents()
                        Call ShowPoleDisplay(Mid(.Item(.Row, 2) & "x" & .Item(.Row, 3), 1, 18), False)
                        Call ShowPoleDisplay(.Item(.Row, 5), True)
                    Case "QTYMinus"
    ...
    Private Sub QtyCalculator_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles QtyCalculator.Click
        If OrderDetail.Row > 0 Then
            ...
            Application.DoEvents()
            Call ShowPoleDisplay(Mid(OrderDetail.Item(OrderDetail.Row, 2) & "x" & OrderDetail.Item(OrderDetail.Row, 3), 1, 17), False)
            '& vbNewLine & "@" & selItemPrice
        ...
        With OrderDetail           
            If .Rows.Count > 1 Then
                ...
                If Len(Trim(.Item(.Row, 9))) > 0 Then ' Col=9 = MBTRANSDTUID item pernah tersimpan dalam database
                        ...
                        If TMPRecord.Item("MBTRANSDTITEMSTAT") > 0 Then
                            ...
                            With UserPermition
                                ...
                                If .DeleteOrderAccess Then
                                    ...
                                    Call MyDatabase.MyAdapter("UPDATE MBTRANSDT SET MBTRANSDTITEMCANCELLEDNOTE='" & ReplacePetik(ItemNotes) & "',MBTRANSDTITEMSTAT=-1,DELETEDUSER='" & UserInformation.UserName & "',DELETEDDATETIME='" & Format(Now, "dd.MM.yyyy, HH:mm:ss") & "' WHERE MBTRANSDTPARENTUID = '" & OrderDetail.Item(OrderDetail.Row, 9) & "'")
                    ...
                    If ShowQuestion(Me, "Hapus item " & Strings.Left(.Item(.Row, 3), IIf(InStr(.Item(.Row, 3), vbNewLine) > 0, InStr(.Item(.Row, 3), vbNewLine) - 1, Len(.Item(.Row, 3)))) & " dari daftar ?") = True Then
                        ...
                        Call MyDatabase.MyAdapter("UPDATE MBTRANSDT SET DELETEDUSER='" & UserInformation.UserName & "' WHERE MBTRANSDTPARENTUID = '" & .Item(.Row, 9) & "'")
                        Call MyDatabase.MyAdapter("DELETE FROM MBTRANSDT WHERE MBTRANSDTPARENTUID = '" & .Item(.Row, 9) & "'")
                        Application.DoEvents()
    ...
    Private Sub SimpanDetailExistingMBNew(ByVal InputMBTransUID As String)
        Dim NEWOrderListCollection As New Collection
        If mbTransNO <> "" Then
    ...
    Private Sub SimpanDetailExistingMB(ByVal InputMBTransUID As String)
        ...
        oldModifiedTime = Format(Now, "dd.MM.yyyy, HH:mm:ss")
        Call MyDatabase.MyAdapter("UPDATE MBTRANS SET MBTRANSDPVAL='" & nilaiDP & "',MODIFIEDDATETIME='" & Format(Now, "dd.MM.yyyy, HH:mm:ss") & "',MBTRANSRSVTRANSUID=" & IIf(tmpRSV = Nothing, "NULL", "'" & tmpRSV & "'") & " WHERE MBTRANSUID='" & InputMBTransUID & "'")
...
#Region "Form Control Function"
    ...
    Private Sub Form_Invoice_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles Me.KeyPress
            ...
            If rs.Read = True Then
                ...
                Else
                    ...
                    If tmpImage = "5" Or tmpImage = "6" Then
                        'AND (SERVICETYPEDEFAULT = 2 OR SERVICETYPEACTV = 0)
    ...
    Private Sub BTNMakeBill_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BTNMakeBill.Click
        ...
        OBJNew.isDelivery = False
        If pubIDCustomer <> Nothing Then
            OBJNew.isDelivery = True
        End If
    ...
    Private Sub OrderDetail_DoubleClick(ByVal sender As Object, ByVal e As System.EventArgs) Handles OrderDetail.DoubleClick
        If OrderDetail.Item(OrderDetail.Row, 10) = "1" Then Exit Sub
        With OrderDetail
    ...
    Private Sub BTNEdit_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BTNEdit.Click
        ...
        If OBJNew.OKStatus = True Then
            ...
            If tmpRSV <> OBJNew.ReservationList.Columns(1).Text Then
                'If isAdaYangDimasak(TransactionUID) = True Then
                '    ShowMessage(Me, "Maaf, nota ini tidak dapat diganti dengan reservasi lain, karena ada menu yang sudah dimasak !")
                '    Exit Sub
                'End If          
...
#End Region
    ...
    Private Sub txtBarcode_MouseDown(ByVal sender As Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles txtBarcode.MouseDown
        ...
        End If
  End Sub
  Public Sub TranfToCustDisplay()
    Dim gEtAktifrOw As Integer
    gEtAktifrOw = OrderDetail.Row
    Dim PlusForModItem As String = ""

    If ItemIdentifyClick = "mod" Then
      ItemuIdFormCustomerDisplay = OrderDetail.Item(gEtAktifrOw, 1).ToString()
      ItemQTYFormCustomerDisplay = OrderDetail.Item(gEtAktifrOw, 2).ToString()
      ItemNameFormCUstomerDisplay = sPlitArt + "(" + pSahpArtB + ")"
      ItemNameFormCUstomerDisplay = ItemNameFormCUstomerDisplay.Replace(vbCr, "").Replace(vbLf, "")
      ItemAmountFormCustomerDisplay = OrderDetail.Item(gEtAktifrOw, 5).ToString()
      PlusForModItem = OrderDetail.Item(gEtAktifrOw, 14).ToString()
      'MessageBox.Show(ItemNameFormCUstomerDisplay)
    Else
      ItemuIdFormCustomerDisplay = OrderDetail.Item(gEtAktifrOw, 1)
      ItemNameFormCUstomerDisplay = OrderDetail.Item(gEtAktifrOw, 3)
      ItemQTYFormCustomerDisplay = OrderDetail.Item(gEtAktifrOw, 2)
      ItemAmountFormCustomerDisplay = OrderDetail.Item(gEtAktifrOw, 5)
      pSahpArt = ""
    End If

    

    ItemMixFormCustomerDisplay = ItemuIdFormCustomerDisplay & "?" & ItemQTYFormCustomerDisplay & "?" & ItemNameFormCUstomerDisplay & "?" & ItemAmountFormCustomerDisplay & "?" & ItemIdentifyClick & "? " & "? "
  End Sub
End Class
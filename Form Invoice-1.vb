...
#Region "Initialize & Object Function"
  ...
  Private Sub SettingAccess(ByVal Access As Boolean)
    ...
    If Access = True Then
      'BTNMakeBill.Enabled = Access:BTNMakeBill.VisualStyle = C1Input.VisualStyle.Office2007Blue
  ...
  Private Sub CategoryClick(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Category1.Click, Category2.Click, Category3.Click, Category4.Click, Category5.Click, Category6.Click, Category7.Click, Category8.Click, Category9.Click, Category10.Click, Category11.Click, Category12.Click
    ...
    If PrefInfo.VDeptIntance_IsUse = False Or Trim(GetFieldValueDBString("USERLOGIN", "USERLOGINDEPTUID", "WHERE USERLOGINUID='" & UserInformation.UserUID & "'")) = "1" Then
      ...
      PMCollection = DBListCollection("SELECT A.INVENUID, A.INVENNO, A.INVENNAME, A.INVENLEVEL, A.INVENTYPEID, A.INVENCATUID, A.INVENPARENTUID, A.INVENMEASUNITUID, A.INVENDFTWAREHUID, A.INVENDISCPERC, A.INVENSALESTAXUID, A.INVENDFTVENDUID, A.INVENPURCHTAXUID, A.INVENMINQTYREORDER, A.INVENDESC, A.INVENDFTPRICELISTLVL," & _
                                      "A.INVENMEASUNITDESC, A.INVENKITCHENUID, A.INVENACTV, A.INVENDISPLAYORDER, A.INVENISPRINTED, B.INVENPRICELISTLVL1,B.INVENPRICELISTLVL2,B.INVENPRICELISTLVL3,B.INVENPRICELISTLVL4,B.INVENPRICELISTLVL5,B.INVENPRICELISTLVL6,B.INVENPRICELISTLVL7,B.INVENPRICELISTLVL8,B.INVENPRICELISTLVL9," & _
                                      "B.INVENPRICELISTLVL10,A.INVENCOLOUR,A.INVENEDITABLEMENU FROM INVEN A INNER JOIN INVENPRICELIST B ON A.INVENUID=B.INVENUID WHERE A.INVENCATUID = '" & sender.tag & "' AND (A.INVENACTV IS NULL OR A.INVENACTV = 0 ) ORDER BY A.INVENNAME")
    Else
      PMCollection = DBListCollection("SELECT A.INVENUID, A.INVENNO, A.INVENNAME, A.INVENLEVEL, A.INVENTYPEID, A.INVENCATUID, A.INVENPARENTUID, A.INVENMEASUNITUID, A.INVENDFTWAREHUID, A.INVENDISCPERC, A.INVENSALESTAXUID, A.INVENDFTVENDUID, A.INVENPURCHTAXUID, A.INVENMINQTYREORDER, A.INVENDESC, A.INVENDFTPRICELISTLVL," & _
                                      "A.INVENMEASUNITDESC, A.INVENKITCHENUID, A.INVENACTV, A.INVENDISPLAYORDER, A.INVENISPRINTED, B.INVENPRICELISTLVL1,B.INVENPRICELISTLVL2,B.INVENPRICELISTLVL3,B.INVENPRICELISTLVL4,B.INVENPRICELISTLVL5,B.INVENPRICELISTLVL6,B.INVENPRICELISTLVL7,B.INVENPRICELISTLVL8,B.INVENPRICELISTLVL9," & _
                                      "B.INVENPRICELISTLVL10,A.INVENCOLOUR,A.INVENEDITABLEMENU FROM INVEN A INNER JOIN INVENPRICELIST B ON A.INVENUID=B.INVENUID WHERE A.INVENCATUID = '" & sender.tag & "' AND (A.INVENACTV IS NULL OR A.INVENACTV = 0 ) AND A.INVENDEPTUID='" & ReplacePetik(UserInformation.UserDeptUID) & "' ORDER BY A.INVENNAME")
    End If
      ...
      With OrderDetail
        For i As Integer = 1 To .Rows.Count - 1
          If .Item(i, 1) = selItemUID Then
            ...
            'Call ShowPoleDisplay(vbNewLine & FormatNumber(.Item(i, 5), 0), True)
            Bypas = True : Exit For
          End If
        Next
        If Not Bypas Then
          ...
          .Row = .Rows.Count - 1
          Application.DoEvents()
          Call ShowPoleDisplay("1x" & selItemName & vbNewLine & selItemPrice, False)
          'Call ShowPoleDisplay(Strings.Left(selItemName, 18), False)
          '& vbNewLine & "@" & selItemPrice
          'Call ShowPoleDisplay(vbNewLine & FormatNumber(selItemPrice, 0), True)
        End If
      End With
      ...
      If .Rows.Count > 1 Then
        Select Case sender.name
          Case "QTYPlus"
            ...
            'ShowPoleDisplay(Mid(.Item(.Row, 2) & "x" & .Item(.Row, 3), 1, 20) & vbNewLine & AddPrefixSpace("@" & .Item(.Row, 4)))
            Application.DoEvents()
            Call ShowPoleDisplay(Mid(.Item(.Row, 2) & "x" & .Item(.Row, 3), 1, 18) & vbNewLine & .Item(.Row, 5), False)
...
#Region "Form Control Function"
  ...
  Private Sub BTNReset_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BTNReset.Click
    ...
    If Authorize = True Then
      Authorize = False
      UserInformation.UserInitialize(UUID, UName, UOrderPoint, UTypeUID, , UTypeUID)
      Call MainPage.StatusBarInitialize()
      Call CheckPermission(UserInformation.UserTypeUID, True)
    End If

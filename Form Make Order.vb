...
#Region "Initialize & Object Function"
    ...
    Else
      ...
      VSql = VSql & " AND A.INVENDEPTUID='" & DeptInfo.DeptUID & "'"
      VSql = VSql & " GROUP BY"
      VSql = VSql & " B.INVENCATUID, B.INVENCATNAME, B.INVENCATLVL"
      VSql = VSql & " , B.INVENCATACTV, B.INVENCATISSALEXTAXINCLUDEDUU"
      VSql = VSql & " , B.INVENCATDISPLAYORDER, B.INVENCATSUBCATEGORYID"
                ...
                For Each OBJ As Object In GroupItem.Controls
                    If TypeOf OBJ Is C1Input.C1Button Then
                        If OBJ.name = "Item" & Format(CurOBJIndex, "00") Then
                            ...
                            End Select
                            'OBJ.tag = CurrentRecord(0) & MY_DELIMITER & CurrentRecord(2) & MY_DELIMITER & CurrentRecord(3) & MY_DELIMITER & CurrentRecord(16) & MY_DELIMITER & selectedPriceLevel & MY_DELIMITER & IIf(IsDBNull(CurrentRecord(34)) = True, "0", CurrentRecord(34)) & MY_DELIMITER & IIf(IsDBNull(CurrentRecord(35)) = True, "0", CurrentRecord(35))
    ...
    Private Sub BTNRemove_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BTNRemove.Click
        ...
        With OrderDetail
            Dim tmpNamaMenu As String = Nothing
            If InStr(OrderDetail.Item(OrderDetail.Row, 3), vbNewLine) <> 0 Then
                Dim arrMenu() As String
                arrMenu = Split(OrderDetail.Item(OrderDetail.Row, 3), vbNewLine)
                tmpNamaMenu = arrMenu(0)
            Else
                tmpNamaMenu = OrderDetail.Item(OrderDetail.Row, 3)
            End If
            If .Rows.Count > 1 Then                
                If .Rows(.Row).Height = 25 Then Exit Sub
                If Len(Trim(.Item(.Row, 9))) > 0 Then
                        ...
                        If TMPRecord.Item("MBTRANSDTITEMSTAT") > 0 Then
                            ...
                            With UserPermition
                                ...
                                If .DeleteOrderAccess Then
                                    ...
                                    'Dim DeleteOrder As String = IIf(InStr(OrderDetail.Item(OrderDetail.Row, 3), vbNewLine) = 0, OrderDetail.Item(OrderDetail.Row, 3), "1")
                                    ...
                                    ShowMessage(Me, "Order pesanan '" & tmpNamaMenu & "' telah dibatalkan !")
                    ...
                    If ShowQuestion(Me, "Hapur order pesanan '" & tmpNamaMenu & "' dari daftar ?") = True Then
                ...
                Else
                    ...
                    If ShowQuestion(Me, "Hapus order pesanan '" & tmpNamaMenu & "' dari daftar ?") = True Then                      
                        For i As Integer = .Rows.Count - 1 To 1 Step -1
                            If Trim(.Item(i, 14)) <> "" And .Item(i, 15) <> "" Then
                                If .Item(i, 15) = .Item(.Row, 14) Then
                                    .Rows.Remove(i)
    ...
    Private Sub SimpanDetailNewMB()
        ...
        For ins = 1 To NEWOrderListCollection.Count
            ...
            'Susilo, 8 Agustus 2014. Diganti karena ada teamviewer
    ...
    Private Sub SimpanDetailExistingMB(ByVal InputMBTransUID As String)
        ...
        '                'ItemRecord.Read()
        '                'Susilo, 8 Agustus 2014. Dirubah karena ada item modifier
